...
export function createReactRenderers(options: RendererOptions = {}) {
  const elementRenderers: Record<string, ElementRenderer> = {
    TextBlock: (element, context) => (
      <TextBlock key={element.id} element={element as any} />
    ),
    Image: (element, context) => (
      <Image key={element.id} element={element as any} />
    ),
    Container: (element, context) => (
      <Container key={element.id} element={element as any} />
    ),
    'Input.Text': (element, context) => (
      <InputText key={element.id} element={element as any} />
    ),
    ActionSet: (element, context) => (
      <ActionSet key={element.id} element={element as any} />
    ),
  };

  const actionRenderers: Record<string, ActionRenderer> = {
    'Action.Submit': (action, context) => (
      <SubmitAction key={action.id} action={action as any} />
    ),
    'Action.OpenUrl': (action, context) => (
      <OpenUrlAction key={action.id} action={action as any} />
    ),
...
export function renderElement(element: CardElement, context: RenderContext): React.ReactElement | null {
  const renderer = context.elementRenderers[element.type];
  if (!renderer) {
    console.warn(`No renderer found for element type: ${element.type}`);
    return (
      <div key={element.id} style={{ padding: '8px', backgroundColor: '#FFF3CD', border: '1px solid #FFC107', borderRadius: '4px' }}>
        Unsupported element: {element.type}
      </div>
    );
  }
  return renderer(element, context) as React.ReactElement;
}

// Helper function to render any action
export function renderAction(action: Action, context: RenderContext): React.ReactElement | null {
  ...
  return renderer(action, context) as React.ReactElement;
}
