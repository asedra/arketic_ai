...
export const Container = ({ element }) => {
    ...
    const { items, style: containerStyle = 'default', verticalContentAlignment = 'top', bleed = false, backgroundImage, minHeight, selectAction, } = element;
    const handleClick = () => {
        if (selectAction) {
            // Haptic feedback
    ...
    const styleConfig = context.hostConfig.containerStyles[containerStyle];
    if (styleConfig) {
    ...
    return (_jsxs("div", { ref: containerRef, className: clsx('adaptive-card__container', `adaptive-card__container--${containerStyle}`, `adaptive-card__container--align-${verticalContentAlignment}`, {
            ...
            'adaptive-card__container--pressed': isPressed,
        }), style: containerStyles, onClick: handleClick, onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseLeave, onMouseDown: handleMouseDown, onMouseUp: handleMouseUp, children: [selectAction && (_jsx("div", { className: "container-hover-overlay", style: {
                    position: 'absolute',
                    top: 0,
                    left: 0,
                    right: 0,
                    ...
                    pointerEvents: 'none',
                    zIndex: 1,
                } })), items.map((item, index) => (_jsx("div", { className: "adaptive-card__container-item", style: {
                    animation: `fadeInUp 0.6s ease-out ${index * 0.1}s both`,
                    position: 'relative',
                    zIndex: 2,
                }, children: renderElement(item, context) }, item.id || `item-${index}`)))] }));
};
