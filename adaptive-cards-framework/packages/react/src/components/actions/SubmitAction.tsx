...
import { ActionSubmit } from '@adaptive-cards/core';
...
export const SubmitAction: React.FC<SubmitActionProps> = ({ action }) => {
  ...
  return (
    <button
      ...
      disabled={!isEnabled || isClicked}
      title={tooltip}
      className={clsx(
        'adaptive-card__action',
        'adaptive-card__action--submit',
    ...
    >
      ...
      {showSuccess ? (
        <>
          ...
          <span className="adaptive-card__action-title">Success!</span>
        </>
      ) : (
        <>
          {iconUrl && (
          ...
          <span className="adaptive-card__action-title">
            {isClicked ? (
