...
import { InputText as IInputText } from '@adaptive-cards/core';
...
export const InputText: React.FC<InputTextProps> = ({ element }) => {
  ...
  const getBorderColor = () => {
    ...
    if (isFocused) return '#0078D4';
    return '#CCCCCC';
  };

  const getBoxShadow = () => {
    if (hasError) return '0 0 0 3px rgba(255, 0, 0, 0.1)';
    if (showSuccess) return '0 0 0 3px rgba(16, 185, 129, 0.1)';
    if (isFocused) return '0 0 0 3px rgba(0, 120, 212, 0.1)';
    return 'none';
  };
  ...
  const inputProps = {
    ...
    className: clsx(
      ...
      'adaptive-card__input--text',
      `adaptive-card__input--${inputStyle}`,
      {
  ...
  return (
    <div className="adaptive-card__input-container" style={{ position: 'relative' }}>
      {label && (
        <label 
          htmlFor={id}
          className={clsx(
        ...
        >
          ...
          {isRequired && (
            <span 
              className="adaptive-card__required-indicator"
              style={{
                color: '#FF0000',
                marginLeft: '4px',
      ...
      {/* Enhanced error messages with friendly tone */}
      {hasError && (
        <div 
          className="adaptive-card__input-error"
          style={{
            marginTop: '8px',
            fontSize: '12px',
            color: '#FF0000',
        ...
        >
          <span>😅</span>
          {errorMessage || (isRequired ? "Oops! This field needs some love." : "Something's not quite right.")}
        </div>
      )}
      ...
      {showSuccess && !hasError && (
        <div 
          className="adaptive-card__input-success"
          style={{
            marginTop: '8px',
            fontSize: '12px',
            color: '#10b981',
