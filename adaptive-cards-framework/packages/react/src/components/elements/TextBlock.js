...
export const TextBlock = ({ element }) => {
    ...
    const { text, color = 'default', size = 'default', weight = 'default', horizontalAlignment = 'left', wrap = false, isSubtle = false, maxLines, } = element;
    const fontSize = context.hostConfig.fontSizes[size] || context.hostConfig.fontSizes.default;
    const fontWeight = context.hostConfig.fontWeights[weight] || context.hostConfig.fontWeights.default;
    const style = {
    ...
    const colorConfig = containerStyle.foregroundColors[color];
    if (colorConfig) {
    ...
    return (_jsx("div", { ref: textRef, className: clsx('adaptive-card__text-block', `adaptive-card__text-block--${color}`, `adaptive-card__text-block--${size}`, `adaptive-card__text-block--${weight}`, {
        ...
        }), style: style, children: shouldTypewrite ? (_jsxs("span", { children: [displayText, currentIndex < text.length && (_jsx("span", { className: "cursor", style: {
